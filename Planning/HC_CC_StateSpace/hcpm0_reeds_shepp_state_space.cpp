#include "hcpm0_reeds_shepp_state_space.h"

class HCPM0_ReedsSheppStateSpace::HCPM0_ReedsShepp
{
public:
    /************************ TT ************************/
    /**
                    ...
                    Configuration **q1, Configuration **q2) const
    {
        ...
        *cstart = new HC_CC_Circle(**q2, !c2.getLeft(), c2.getForward(), HC_REGULAR, _parent->_hc_cc_circle_param);
        ...
        *q1     = new Configuration(c1.getStart().getX(), c1.getStart().getY(), c1.getStart().getPsi(), c1.getKappa());
        return (*cstart)->hc_turn_lenght(**q1) + (*cend)->cc_turn_lenght(**q2);
    }
                     ...
                     HC_CC_Circle **cstart, HC_CC_Circle **cend, Configuration **q) const
    {
        ...
        *cend   = new HC_CC_Circle(c2);
        return (*cstart)->rs_turn_lenght(**q) + (*cend)->hc_turn_lenght(**q);
    }
    ...
    bool TTcT_Exist(HC_CC_Circle &c1, HC_CC_Circle &c2) const
    {
        ...
        {
            return (_distance <= 2 * ( c2.getRadius() + fabs(c2.getKappaInv()) ))
                && (_distance >= 2 * ( c2.getRadius() - fabs(c2.getKappaInv()) ));
        }
    }
    ...
    {
        double psi = _angle;
        double r1 = 2 * c2.getRadius();
        double r2 = 2 * fabs(c2.getKappaInv());
        double delta_x = (pow(r1, 2) + pow(_distance, 2) - pow(r2, 2)) / ( 2 * _distance );
        double delta_y = sqrt(pow(r1, 2) - pow(delta_x,2));
        double x,y;
    ...
    bool TiST_Exist( HC_CC_Circle &c1, HC_CC_Circle &c2 ) const
    {
        ...
        {
            return (_distance >= 2 * c2.getRadius());
        }
    }
    ...
    bool TeST_Exist( HC_CC_Circle &c1, HC_CC_Circle &c2 ) const
    {
        ...
        {
            return (_distance >= 2 * c2.getRadius() * c2.getSinMu());
        }
    }
    ...
    bool TiSTcT_Exist( HC_CC_Circle &c1, HC_CC_Circle &c2 ) const
    {
        ...
        {
            return (_distance >= sqrt(pow(2 * c2.getRadius() * c2.getSinMu() + 2 *fabs(c2.getKappaInv()), 2) +
                                      pow(2 * c2.getRadius() * c2.getCosMu(), 2)));
        }
    }
    ...
    bool TeSTcT_Exist( HC_CC_Circle &c1, HC_CC_Circle &c2) const
    {
        ...
        {
            return (_distance >= 2 * (fabs(c2.getKappaInv()) + c2.getRadius() * c2.getSinMu()));
        }
    }
                        ...
                        HC_CC_Circle **ci) const
    {
        ...
        *cend   = new HC_CC_Circle(c2);
        *q1     = new Configuration(c1.getStart().getX(), c1.getStart().getY(), c1.getStart().getPsi(), c1.getKappa());
        *ci     = new HC_CC_Circle(**q3, !c2.getLeft(), c2.getForward(), true, _parent->_hc_cc_circle_param);
        return  (*cstart)->hc_turn_lenght(**q1) +
                (**q2).distance(**q3) +
                (*ci)->hc_turn_lenght(**q4) +
                (*cend)->hc_turn_lenght(**q4);
    ...
    bool TcTiSTcT_Exist( HC_CC_Circle &c1, HC_CC_Circle &c2 ) const
    {
        ...
        {
            return (_distance >= sqrt(pow(2 * c2.getRadius(), 2) +
                                      16 * c2.getRadius() * c2.getSinMu() * fabs(c2.getKappaInv()) +
                                      pow(4 * c2.getKappaInv(), 2)));
        }
    }
    ...
    bool TcTeSTcT_Exist( HC_CC_Circle &c1, HC_CC_Circle &c2) const
    {
        ...
        {
            return (_distance >= 4 * fabs(c2.getKappaInv()) + 2 * c2.getRadius() * c2.getSinMu() );
        }
    }
    ...
    bool TTcTT_Exist(HC_CC_Circle &c1, HC_CC_Circle &c2) const
    {
        ...
        {
            return (_distance <= 4 * c2.getRadius() + 2 * fabs(c2.getKappaInv()));
        }
    }
                              ...
                              Configuration **qb1, Configuration **qb2, Configuration **qb3) const
    {
        ...
        if( _distance < 4 * c2.getRadius() - 2 * fabs(c2.getKappaInv()) )
        {
    ...
    bool TcTTcT_Exist(HC_CC_Circle &c1, HC_CC_Circle &c2) const
    {
        ...
        {
            return (_distance <= 4 * fabs(c2.getKappaInv()) + 2 * c2.getRadius()) &&
                   (_distance >= 4 * fabs(c2.getKappaInv()) - 2 * c2.getRadius());
        }
    }
    ...
    explicit HCPM0_ReedsShepp(HCPM0_ReedsSheppStateSpace *parent)
    {
        _parent = parent;
    }
...
private:
    HCPM0_ReedsSheppStateSpace *_parent;

    double _distance = 0.0;
    double _angle = 0.0;
};// end of HCPM0_ReedsShepp class function define
 ...
 */
HCPM0_ReedsSheppStateSpace::HCPM0_ReedsSheppStateSpace(double kappa, double sigma, double discretization)
    : HC_CC_StateSpace(kappa, sigma, discretization)
    , _hcpm0_reeds_shepp{ unique_ptr<HCPM0_ReedsShepp>(new HCPM0_ReedsShepp(this))}
{
 ...
 * @brief Destructor
 */
HCPM0_ReedsSheppStateSpace::~HCPM0_ReedsSheppStateSpace() = default;
